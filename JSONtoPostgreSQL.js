...
try {
  ...
  fs.readFile("listings.json", "utf8", function (err, data) {
    ...
    // Iterate through entries and add them to Listing
    const entries = listingData.entries;
    let i = 0;
    while (i < entries.length) {
      const temp = Listing.create({
        code: entries[i].code,
        name: entries[i].name,
        coordinates: JSON.stringify(entries[i].coordinates),

...
async function retrieveAllListings() {
  ...
  // select without the useless attributes
  const buildings = await Listing.findAll({
    attributes: { exclude: ["createdAt", "updatedAt", "id"] },
  });
  // Iterate through them to print
  let i = 0;
  while (i < buildings.length) {
    const cur = buildings[i++];
    console.log(cur.dataValues);
  }
}
/* 
...
async function findLibraryWest() {
  ...
  // Print result
  const cur = results[0];
  console.log("Library West Values:");
  console.log(cur.dataValues);
}
...
async function removeCable() {
  ...
  let val = await Listing.findAll();
  console.log("Previous size is " + val.length);
  // Delete code
  deleteByCode("CABL");
  // Verify the updated size
  val = await Listing.findAll();
  console.log("Removed CABL current size is now " + val.length);
}

// Generalized delete function
async function deleteByCode(val) {
...
async function addDSIT() {
  //ADD CODE HERE
  console.log(
    "\nAdding the new DSIT BLDG that will be across from Reitz union. Bye Bye CSE, Hub, and French Fries."
  );
...
async function updatePhelpsLab() {
  ...
  // Get and print result to console
  const result = await Listing.findAll({
    where: {
      code: "PHL",
    },
...
console.log(
  "Use these calls to test that your functions work. Use console.log statements in each so you can look at the terminal window to see what is executing. Also check the database."
);

// Sleep to prevent too many connections at once.
function sleep(ms) {
